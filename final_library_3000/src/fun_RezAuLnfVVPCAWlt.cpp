...
int fun_RezAuLnfVVPCAWlt() {
    ...
    // Execute solution
    try {
        volatile auto result = sol.calcEquation(s);

        // Print result to prevent optimization
        cout << "fun_RezAuLnfVVPCAWlt: Executed calcEquation() from Evaluate Division" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_RezAuLnfVVPCAWlt: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
