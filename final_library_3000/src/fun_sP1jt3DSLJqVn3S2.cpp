...
int fun_sP1jt3DSLJqVn3S2() {
    ...
    // Execute solution
    try {
        volatile auto result = sol.isRowSafe(testValue);

        // Print result to prevent optimization
        cout << "fun_sP1jt3DSLJqVn3S2: Executed isRowSafe() from Sudoku  Solver" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_sP1jt3DSLJqVn3S2: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
