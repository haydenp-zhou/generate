...
int fun_8rotRbdkBRz5sO20() {
    ...
    // Execute solution
    try {
        volatile auto result = sol.minExtraChar(nums);

        // Print result to prevent optimization
        cout << "fun_8rotRbdkBRz5sO20: Executed minExtraChar() from Extra Characters In A String" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_8rotRbdkBRz5sO20: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
