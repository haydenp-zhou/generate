...
int fun_OVDQNoMSg2nPanix() {
    ...
    // Execute solution
    try {
        volatile auto result = sol.valueAfterKSeconds(nums);

        // Print result to prevent optimization
        cout << "fun_OVDQNoMSg2nPanix: Executed valueAfterKSeconds() from Find The N Th Value After K Seconds" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_OVDQNoMSg2nPanix: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
