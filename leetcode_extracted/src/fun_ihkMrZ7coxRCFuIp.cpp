...
int fun_ihkMrZ7coxRCFuIp() {
    ...
    // Execute solution
    try {
        volatile auto result = sol.addDigits(n);

        // Print result to prevent optimization
        cout << "fun_ihkMrZ7coxRCFuIp: Executed addDigits() from Add Digits" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_ihkMrZ7coxRCFuIp: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
