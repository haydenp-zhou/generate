...
int fun_rlcq71aShEr3OqmR() {
    // LeetCode Problem: Shortest  Common  Supersequence
    ...
    // Execute solution
    try {
        volatile auto result = sol.shortestCommonSupersequence(s);

        // Print result to prevent optimization
        cout << "fun_rlcq71aShEr3OqmR: Executed shortestCommonSupersequence() from Shortest  Common  Supersequence" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_rlcq71aShEr3OqmR: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
