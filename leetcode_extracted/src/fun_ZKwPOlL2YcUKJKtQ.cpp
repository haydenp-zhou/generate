...
int fun_ZKwPOlL2YcUKJKtQ() {
    ...
    try {
        ...
        cout << "fun_ZKwPOlL2YcUKJKtQ: Executed waysToSplit() from Ways To Split Array Into Three Subarrays" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_ZKwPOlL2YcUKJKtQ: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
