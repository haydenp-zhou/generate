...
int fun_8uq1LoGiltXxCCNF() {
    ...
    // Execute solution
    try {
        volatile auto result = sol.maxNumber(nums);

        // Print result to prevent optimization
        cout << "fun_8uq1LoGiltXxCCNF: Executed maxNumber() from Create Maximum Number" << endl;

        volatile int status = 1;
        return const_cast<const int&>(status);

    } catch (const exception& e) {
        cout << "fun_8uq1LoGiltXxCCNF: Exception during execution: " << e.what() << endl;
        return 0;
    }
}
